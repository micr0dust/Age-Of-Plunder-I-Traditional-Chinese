...
execute @a ~ ~ ~ /function setting:var if @a[score_AIset=0,score_AIset_min=0]
...
execute @a[score_gamemode=0,score_gamemode_min=0] ~ ~ ~ /summon minecraft:shulker 105 30 -135 {Color:11,CustomName:"指揮中心--西班牙",CustomNameVisible:1,NoAI:1b,Attributes:[{Name:generic.maxHealth,Base:200}],Health:200.0f,Tags:["blue1"],Team:blue}
setblock 51 20 -170 minecraft:air
tp @a[team=blue] 110 26 -127
tp @a[team=red] 41 25 -216
execute @a[score_gamemode=1,score_gamemode_min=1] ~ ~ ~ /clone -2 34 -96 6 41 -88 -2 43 -96
execute @a[score_gamemode=0,score_gamemode_min=0] ~ ~ ~ /clone 8 34 -96 16 41 -88 -2 43 -96
...
scoreboard players set @a money 60
scoreboard players set @a count 1
function ai:chat/join
execute @a[score_gamemode=1,score_gamemode_min=1] ~ ~ ~ /gamerule gameLoopFunction ai:mony_test
